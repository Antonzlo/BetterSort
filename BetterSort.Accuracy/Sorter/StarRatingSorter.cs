...
namespace BetterSort.Accuracy.Sorter {
  ...
  public class StarRatingSorter(SiraLog logger, IAccuracyRepository repository) : ISortFilter {
    ...
    public void NotifyChange(IEnumerable<ILevelPreview>? newLevels, bool isSelected = false) {
      try {
        ...
        _logger.Debug($"{nameof(StarRatingSorter)}.{nameof(NotifyChange)}: newLevels.Count: {newLevels.Count()}, isSelected: {isSelected}");
    ...
    internal static SortResult SortInternal(IEnumerable<ILevelPreview>? levels, Func<SorterData?> getRecords, List<LevelRecord> mapping) {
      ...
      var comparer = new LevelStarRatingComparer(records);
      ...
      var legend = StarRatingLegendMaker.GetLevelLegend(mapping, ordered.Count);
